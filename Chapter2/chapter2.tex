...
The input sequences are present as walks in the graph.
But as the complexity of the graph grows the number of paths will increase exponentially.
We could add transition weights between nodes relating the frequency of that particular node pairing.
While weighted random walks in the graph will have local similarity to the graph's preimage, they are unlikely to provide a good model for the input sequences.
Sequences generated from an Markovian process of this type will exhibit an exponential decay in mutual information between their symbols, while natural sequences tend to exhibit power-law decay in mutual information \cite{lin2017critical}.
The only way to be sure that we model the input sequence space is to maintain a record of it in the context of the graph.
...
A limited form of this translation is a \emph{genotype}, which maps the implied bubble formed across multiple copies of a homologous locus in one genome into the space of the graph.
...
We expect these haplotype paths to be fully embedded in the graph, so they can be represented as a walk through nodes.
...
We can optionally compact series of nodes (which represent single characters) where no bifurcations occur to obtain a compacted graph.
...
{\tt vg}'s design largely preceded its development.
...
As mentioned before, the GFA format can be used to directly encode VGs, although some default fields (particularly the overlap CIGAR on edges) are generally not useful and carry no information.
...
The graph is likely to remain static during many kinds of analysis, and so we have the opportunity to compress it into static data structures that provide efficient access to important aspects of the graph with low memory overhead.
...
In the context of a short read aligner, such losses may be undesirable as long as there is sufficient memory to load the entire index into system memory, and so I have tuned the index by choosing compression strategies appropriate for its use on current datasets.
...
To reduce redundancy, GCSA2 uses a \emph{pruned} DBG by using nodes with labels shorter than $k$ where these shorter strings still uniquely identify the beginning of their corresponding paths in the unpruned DBG.
...
Impressively, GCSA2 uses around 1 bit per $k$-mer in indexes of the 1000GP pangenome graph for $k=128$, which is favorable with comparison to true DBG indexes, as these need to record information about each $k$-mer that is not important for GCSA2.
...
Provided the final alignment is completed against the full VG using DP-based alignment techniques that have non-exponential costs, the DBG input to GCSA2 may be constructed from a VG in which complex regions have been masked or reduced in local complexity using prior information such as a haplotype panel.
...
Unfortunately, this approach is challenging due to the extremely large size of $T$ for moderately-sized haplotype sets embedded in variation graphs ($|T| \approx 10^{12}$ for the 1000GP \cite{siren2018haplotype}).
...
If we sort the identifier space of these graphs, then most haplotypes passing through the graph will consist of increasing or decreasing sequences.
...
These features suggest that a local compression of each node's context in the graph can be achieved very efficiently.
...
Nodes tend to have low degree, which means the local alphabet size $|\Sigma_i|$ is small, and we can afford to decompress a small local alphabet encoding efficiently.
...
However, performance was far worse and the initial version of the aligner based on these systems could not achieve meaningful results using reasonable amounts of time for large graphs.
...
Finally, on the networked storage available in my institutional setting, the construction costs for disk-backed index models were usually much worse than those of the XG and GCSA2 models.
...
Each chunk is aligned independently using one of several possible algorithms.
I use GCSA2 and the XG index to find and cluster maximal exact matches (MEMs) using a global pseudoalignment algorithm that exploits approximate distance indexes in the graph to drive collinear chaining of MEMs.
The same collinear chaining model, with different parameters, is used to establish the optimal global chain through the alignment chunks, thus yielding a full alignment for the input sequence of any size.
...
Although the exact algorithm is different, in spirit our implementation is similar to that developed in \cite{kuosmanen2018using}, which extends collinear chaining to DAGs by running a similar model over the paths in a minimum path cover which they efficiently compute on the graph.
...
However, our partial order alignment implementation does not handle these structures, and so when they occur we convert them into a $k$-path equivalent acyclic graph in order to align with DAG-based alignment algorithms.
...
We record the maximum scores of alignments between $Q$ and $G$ in the tensor $\hat{\cal H} = {\cal H}_1 \ldots {\cal H}_{|N|} :$ each ${\cal H}_i$ is a $|seq(n_i)| \times |Q|$ matrix.
...
Suzki had already implemented a version alignment over graphs by transforming the graph into a tree through a dynamic unrolling process akin to that described in \ref{sec:DAGify} and aligning to the tree using libgaba\footnote{\url{https://github.com/ocxtal/comb}}.
His implementation supports graph to graph alignment as described in section \ref{sec:translation}, but the exponential expansion of the alignment problem is fundamentally limiting.
