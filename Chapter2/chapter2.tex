...
This index formed the basis for the first prototypes of the {\tt vg} mapper, but is be impractical due to its time and memory requirements.
...
It is the first index to provide an emulation of the full range of suffix tree operations in the context of a graph.
...
\caption[The MEM Chain Model]{
  ...
  Each node refers to a single MEM, its positions in the graph, and the score derived from the first pass of $\textsc{MaxSum}(G_\textsc{memchain})$.
  ...
  The maximum scoring chain (in red) is found by the application of a standard max-sum dynamic programming approach, as described in the text.
}
