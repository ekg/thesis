...
\DIFaddbegin \begin{figure}[htbp!]
  ...
  \caption[The basic elements of a variation graph]{
    ...
    (A) shows the variation graph constructed by progressive alignment of three sequences labeled }\emph{\DIFaddFL{a}}\DIFaddFL{, }\emph{\DIFaddFL{b}}\DIFaddFL{, and }\emph{\DIFaddFL{c}}\DIFaddFL{.
    ...
    (D) shows the path }\emph{\DIFaddFL{a}} \DIFaddFL{embedded in the graph in full. Other paths are not shown, but are recorded similarly.
...
compress the BWT using encodings that allow fast decompression or direct operation on the compressed data (compressed for example with run-length compression, or that of Raman, Raman, and Rao \mbox{%DIFAUXCMD
...
In }{\tt \DIFadd{vg}}\DIFadd{, I was ultimately to encourage the development of and use a practical implementation of a succinct data structure akin to the CSA, the GCSA2.
This model was developed within the context of }{\tt \DIFadd{vg}}\DIFadd{, and is the first sequence index specifically designed to work on variation graphs.
...
Our research focus on the replication of results of the short read mapper }\DIFdelend \DIFaddbegin \DIFadd{the index could be efficiently compressed, but still the graph for the human 1000 Genomes Project required over 200GB of memory, and was too slow to use unless cached in main memory.
This index formed the basis for the first prototypes of the }\DIFaddend {\tt \DIFdelbegin \DIFdel{bwa mem}\DIFdelend \DIFaddbegin \DIFadd{vg}\DIFaddend } \DIFdelbegin \DIFdel{, which seeds alignments with maximal exact matches (MEMs), encouraged a similar approach for }\DIFdelend \DIFaddbegin \DIFadd{mapper, but is impractical due to its time and memory requirements.
}
...
\DIFadd{We can iteratively apply $LF$ mapping to execute search on the BWT.
...
\DIFadd{In contrast to $k$-mer based indexes, generalizations of the CSA to graphs have required substantial conceptual development to produce.
...
The MSA is converted into }\DIFaddend a reverse deterministic automaton \DIFdelbegin \DIFdel{rather than }\DIFdelend \DIFaddbegin \DIFadd{via a standard procedure for determinizing finite state automata.
...
%DIF > The GCSA is also an important basis for the GCSA2 model used in {\tt vg}, but for space reasons I will not provide an exhaustive description of it.
...
\DIFadd{Sir\'{e}n's update to GCSA, GCSA2, is specifically designed for application to arbitrary variation graphs \mbox{%DIFAUXCMD
\cite{siren2017indexing}}\hspace{0pt}%DIFAUXCMD
.
...
Shorter graph path queries are fully sufficient as the basis for sequence mapping to the graph, which is the primary application of }\DIFaddend a \DIFdelbegin \DIFdel{DBG.
Impressively, }\DIFdelend \DIFaddbegin \DIFadd{graph sequence index.
}

\DIFaddend GCSA2 \DIFdelbegin \DIFdel{uses around 1 bit per }\DIFdelend \DIFaddbegin \DIFadd{brings together ideas from succinct DBGs and the GCSA model.
At a high level, we can consider it to be a compact de Bruijn graph }\DIFaddend $k$\DIFdelbegin \DIFdel{-mer in indexes of the 1000GP pangenome graph for $k=128$, which is favorable with comparison to true DBG indexes.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{As an FM-index like structure, }\DIFdelend \DIFaddbegin \DIFadd{-mer index of a variation graph.
...
}\DIFaddend GCSA2 \DIFdelbegin \DIFdel{supports queries for a pattern $X$ that yield the suffix array interval matching a given pattern: $find(X) = ( sp_{X}, ep_{X} )$, }\DIFdelend \DIFaddbegin \DIFadd{transforms the pruned DBG into a BWT, using additional bitvectors $IN$ and $OUT$ analogous to $F$ }\DIFaddend and \DIFdelbegin \DIFdel{$locate( sp_{X}, ep_{X} ) = \{ b_1 \ldots b_{count(sp_{X}, ep_{X})} \}$ which yields the positions in the input VG where the given patterns occur.
The LCP array allows the index to support several operations that require a suffix tree, including $count(sp_i, ep_i)$, which returns the number of matches for a given suffix array range, and $parent(sp_i, ep_i) = (sp_j, ep_j)$, which allows us to traverse the suffix links embedded in the suffix tree and forms the basis of maximal exact match (MEM) inference using GCSA2}\DIFdelend \DIFaddbegin \DIFadd{$L$ in SDBG to record the graph topology in a generalization of the FM-index.
The full data structure includes extensions to allow suffix tree operations that are important for finding maximal exact matches during sequence search}\DIFaddend .
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend GCSA2 \DIFdelbegin \DIFdel{resolves a number of problems that limit the utility of other graph path indexing schemes.
...
It }\DIFdelend \DIFaddbegin \DIFadd{any variation graph, including those that have cycles or other regions of topological complexity, with the caveat that we cannot search for sequences of length greater than $k$ without the risk of finding false positives.
...
\DIFadd{The GCSA2 encoding is highly efficient on real data sets.
...
These }\DIFdelend \DIFaddbegin \DIFadd{reseeding'' passes whereby long MEMs are used as the basis for further exact match finding, but with a maximum limit to the detected match size.
...
\DIFdelbegin \DIFdel{By masking out the states in this path and re-running the Viterbi algorithm on the model, we can extract }\DIFdelend \DIFaddbegin \DIFadd{Transitions between MEMs may encode gaps or mismatches.
...
\DIFaddendFL \caption[The \emph{dozeu} X-drop alignment algorithm]{
  \DIFdelbeginFL \DIFdelFL{The }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Evaluation of the }\DIFaddendFL \emph{dozeu} X-drop alignment algorithm \DIFaddbeginFL \DIFaddFL{for an example graph}\DIFaddendFL .
  \DIFdelbeginFL \DIFdelFL{Black rectangles represent the evaluated region of the score matrix, which each box corresponding to a SIMD vector value.
  Grey }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Gray }\DIFaddendFL cells represent the part of the score matrix for which our X-drop parameter would allow evaluation if we were calculating the matrix one cell at a time.
  \DIFdelbeginFL \DIFdelFL{Reprinted }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{8-cell wide black rectangles that contain the full set of gray cells represent the region of the score matrix for which we calculate scores when using a SIMD-based accelerated version of the algorithm.
...
\DIFaddbegin \DIFadd{Although the development of $G_\textsc{AlignChain}$ is very similar to $G_\textsc{MemChain}$, a number of important differences distinguish the two models.
...
This transformation is used to project graph based alignments onto the linear reference, and is of great utility in the application of }{\tt \DIFadd{vg}} \DIFadd{in resequencing based analyses, where it supports a lossy translation between }{\tt \DIFadd{vg}}\DIFadd{'s native GAM format and BAM.
}
...
\caption[Force-directed layout with Graphviz's {\tt neato}]{
  A larger region of the same variation graph in figure \ref{fig:vg_view_dot} rendered using {\tt neato\DIFaddbeginFL \DIFaddFL{.}}
    \DIFaddFL{Here, each path in the variation graph is represented by a colored path, with the node steps 
    ...
    Note that the graphviz data input here rendered by }{\tt \DIFaddFL{neato}\DIFaddendFL } \DIFaddbeginFL \DIFaddFL{is the same as that given to }{\tt \DIFaddFL{dot}} \DIFaddFL{in \ref{fig:vg_view_dot}}\DIFaddendFL .
}
...
If we can construct a graph which embeds all the sequences of all genomes which we are interested in, we \DIFdelbegin \DIFdel{resolves }\DIFdelend \DIFaddbegin \DIFadd{resolve }\DIFaddend the separation between reference sequence and variation that is present in standard resequencing.
