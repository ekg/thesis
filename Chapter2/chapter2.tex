...
Algorithm \ref{alg:bwt_find} defines the function $find(P)$, which returns the suffix array interval in which suffixes are prefixed by $P$, should it exist, and an empty interval if $P$ is not a substring of $T$.
...
At this point, it is possible to enumerate and sort the suffixes of the given automaton, build the corresponding BWT, and augment the data structure with an additional bitvector $M$ that records the number of outgoing edges at each node in the automaton.
Backward searching in this structure uses a similar principle to the XBW and succinct DBG models, with select queries on $M$ used to expand the search interval as search progresses through furcations in the graph.
