...
This traceback encodes the alignment, which is most-simply represented as a vector of pairs of homologous bases in each sequence.
...
The algorithm, further refined by Gotoh \cite{gotoh1982improved}, is today one of the most important in genome analysis.
Numerous works over the subsequent decades have offered improved implementations, using vectorized instructions to improve the runtime of the algorithm \cite{farrar2007striped} and heuristics to selectively evaluate only part of the DP matrix \cite{suzuki2017acceleration}.

$O(MN)$ algorithms for sequence alignment are impractical when either $M$ or $N$ becomes large.
So, as sequence databases grew and the size of sequenced genomes increased, heuristic strategies to efficiently reduce the alignment problem size were introduced.
