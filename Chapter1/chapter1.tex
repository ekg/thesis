...
In these approaches, the reference system becomes a \emph{pangenome}.
The reference thus contains all useful prior information required in the current analysis.

Pangenomics is by no means a new concept, and has arisen several times in bioinformatics.
Historically, most interest in this conceptual model has been limited to microbiology, where smaller, simpler, and dramatically more plastic genomes led to the need to consider all genomes together, in an aggregate fashion.
But in recent years the concept has gained traction as a general basis for genome inference.
The applicability of the idea is wide, as few experiments in genomics operate on a single genome.
Almost all work requires comparison and contextualization of inferred genomes.

%far before the issue became the most pressing for those analyzing large eukaryotic genomes.
%but until second generation sequencing allowed the exploration the genomes of
