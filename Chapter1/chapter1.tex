...
The \emph{alignment graph}, first introduced in the context of multiple sequence alignment \cite{kececioglu1993maximum}, represents a collection of alignments in graphical form, allowing an induction of a sequence graph (or MSA when such a graph is partially ordered).
...
Similarly, if we rendered the resulting graph as a bidirectional sequence graph then each connected component in $G_a$ would yield a single base in the sequence space of the graph, but it is worth noting that the equivalence is not exact unless we generalize the alignment graph to represent alignments in both the forward or reverse complement orientations.

The Enredo graph model, used by the Enredo/Pecan multiple genome aligner \cite{paten2008enredo}, generalizes the alignment graph model to bidirectionality by representing each genome segment in the graph with two nodes, a head and tail.
Each segment between a head and tail represents a locus found in one or more locations in the input sequences, and within these segments Pecan is applied to generate colinear alignments.

%
%Summarize work in \cite{paten2011cactus}, as this can be seen as representing a ``third way'' between DBG and string graph or breakpoint graphs.
DBGs may be seen as a kind of intermediate thingy.

% notes
The compressiveness of the resulting graph and its expression of homology relationships in the input sequences are completely dependent on the aligner that was used to build the alignment set.

Much existing effort has focused on algorithms to remove alignment artifacts from the graph and adjust its structure to better match evolutionary relationships.

They can be used to produce ideal input to the methods I describe.
However, in practice they require the availability of high-quality reference genomes as input.
Multiple genome alignment graphs are used to describe sequence relationships in an evolutionarily-compatible fashion, as to allow their use as a basis for evolutionary inference.
...
The authors report an extension of the Needleman-Wunsch alignment algorithm that allows alignment against the graph wherein the graph traversal is converted into a tree of alignments by duplicating the alignment process at each bifurcation.
This alignment algorithm is exponential in the average number of bifurcations per sequence base, which may contribute to the observation that it runs hundreds of times slower than standard alignment methods on 100bp reads \cite{liu2016debga}.
...
\subsubsection{Realigning reads to a local sequence DAG}
...
%\cite{dilthey2015improved}
% this basically just infers the underlying haplotypes given a partially ordered DBG kind of model, then aligns against them
% downstream analysis is pretty much standard
% limited to particular small loci

\subsubsection{Bwbble}

% a fun way of encoding the VCF data into a sequence index
% add a bunch of additional sequences and remember when they are overlapping to fix up the resulting alignments
%\cite{huang2013short}
...
HISAT2 implements variation-aware alignment against human genome scale graphs, including alignment to RNA splicing junctions.
To find seeds globally, it uses an implementation of Jouni Sir\"{e}n's generalized compressed suffix array (GCSA) \cite{siren2011indexing}, which generalizes the FM-index to sequence DAGs, and builds its index including common short SNPs and indels.
...
Although more limited in scope than my work, HISAT2 is an important point of reference as another scalable and mature implementation of sequence to graph alignment, and is the first widely-used aligner based on the GCSA index.
...
\begin{comment}
Where these methods are used in resequencing, they universally project alignments into a linear reference space and standard data formats for further processing.
They do not use the pangenome as the reference itself, but as an of additional source of information.
I found this perspective limited, as it specializes the models used which tends to isolate the algorithms implemented on top of them from integration into a larger ecosystem of tools.
The work I present in this thesis responds to this limitation.
{\tt vg} is not the first pangenomic aligner, but it is the first that models its results in terms of the pangenome itself.
\end{comment}
