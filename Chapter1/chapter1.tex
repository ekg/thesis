...
One interesting feature of these graphs is that their efficient genesis tends to yield a representation in which sequences attached to nodes partially overlap with the nodes in their neighborhood in the graph.
...
In the case of DBGs and the FM-index based string graph assemblers, an iterative pairwise sequence comparison where unitigs (unbranching sequences in the graph) are inferred from the compressed sequence graph yields a similar overlap graph output.
...
The overlaps add complexity to the use and representation of these graphs by differentating them from regular languages.
...
In general this overlap representation makes it difficult to work directly with such graphs using the algorithms we will introduce, but these graphs remain relevant to my work as they may be reduced any overlap graph into a ``blunt-ended'' bidirectional graph using relatively efficient algorithms.

% mention miniasm and friends, bit of grounding
...
% define DBG clearly, show why it is a special case of overlap graph in which overlaps can be computed implicitly
% list all the methods in DBG space
