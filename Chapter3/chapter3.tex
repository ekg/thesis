...
\subsection{Cactus yeast variation graph}
%*0.5p 0.5h*
...
To test the ability of {\tt vg} to use graphs of complex topology, we constructed a variation graph from the whole genome alignment of \emph{de novo} assemblies produced from PacBio sequencing of seven strains of \emph{S. cerevisiae}.
To build this graph, each chromosome of each assembly in \cite{yue2017contrasting} was aligned using LASTZ \cite{harris2007improved} according to a phylogenetic guide tree.
The resulting alignment set was reduced to a variation graph using the Cactus reference-free whole genome multiple alignment method \cite{Paten:2011fva}, which internally maintains a sequence graph equivalent to a VG.
Its output was then converted to {\tt vg} format using the hal2vg utility\footnote{\url{https://github.com/ComparativeGenomicsToolkit/hal2vg}}.
...
This illustrates the ability of {\tt vg} to represent paths corresponding to both collinear (inset) and structurally rearranged (main figure) regions of genomic variation.
...
I compared four variation graphs: a linear reference graph from the standard S288c strain, a linear reference from the SK1 strain, a pangenome graph of all seven strains, and a ``drop SK1'' variation graph in which all sequence private to the strain SK1 was removed from the pangenome graph.
The multiple genome graphs were based on that first constructed with Cactus, as described above, and then filtered down to the various subgraphs using path subsetting facilities in {\tt vg mod}.

\begin{figure}[htbp!]
  \centering
  \includegraphics[width=1.0\textwidth]{Chapter3/Figs/e68fc338_test_sim_yeast_cactus-roc.pdf}
  \caption[Cactus yeast simulation]{Mapping short reads with {\tt vg} to yeast genome references.
    ROC curves obtained by mapping 100,000 simulated SK1 yeast strain 150bp paired-end reads against a variety of references described in the text.
    Reprinted from \cite{garrison2018variation}.}
  \label{fig:cactus_yeast_sim}
\end{figure}


We simulated 100,000 150bp paired-end reads from the SK1 reference, modeling sequencing errors, and mapped them to the four references (ROC curves, Figure \ref{fig:cactus_yeast_sim}).
...
\caption[\emph{S. cerevisiae} variation graphs]{
  ...
  The Illumina data from \cite{yue2017contrasting} was used to build the minia unitigs and minia contigs graphs, while the whole genome, chromosome-resolved PacBio assemblies from the same work were used to build the Cactus and {\tt vg msga} variation graphs.
...
The SGD\_2010 reference contains the mitochondria and 6kbp plasmid sequence, while the S288c assembly excludes them due to the sequencing protocol, where size selection in the library preparation stage removed these short sequences, so to enable direct comparison in later tests these were added to the S288c graph.
...
Where the Cactus variation graph uses a phylogenetic guide tree to structure its construction, {\tt vg msga} simply aligns the chromosomes in order from longest to shortest to the growing graph.
...
We can see this in figure \ref{fig:yeast_bandage}, where the Cactus variation graph (\ref{subfig:cactus_yeast_bandage}) shows two dense repeat structures in its core connected by loops of unique sequence, while the {\tt vg msga} graph appears to have much longer loops, with collapsed repeats embedded in these loops.
...
\begin{figure}[htbp!]
  ...
  \begin{subfigure}[t]{0.85\textwidth}
    ...
    \caption{Cactus variation graph} \label{subfig:cactus_yeast_bandage}
  \end{subfigure}
  \begin{subfigure}[t]{0.85\textwidth}
    \includegraphics[width=1.0\textwidth]{Chapter3/Figs/vg_msga_yeast.png}
    \caption{{\tt vg msga} variation graph} \label{subfig:msga_yeast_bandage}
  \end{subfigure}
...
For each of the seven graphs I aligned a set of 43,337 Pacific Biosciences SK1 reads (mean length 4.7kbp) from \cite{yue2017contrasting} to the graph.
...
\begin{figure}[htbp!]
  ...
  \caption[Long read alignment against various \emph{S. cerevisiae} pangenome graphs]{Density plots of alignment identity when mapping 43,337 Pacific Biosciences long reads from the SK1 strain to different variation graphs.
    ...
    (Top right) the Cactus variation graph vs. the S288c assembly.
    (Bottom right) the {\tt vg msga} variation graph vs. the S288c assembly.
  }
...
For a species such as human, with only 0.1\% nucleotide divergence on average between individual genome sequences, over 90\% of 100bp reads will derive from sequence exactly matching the reference.
...
The full {\tt vg} graph uses 3.92 GB when serialized to disk, and contains 3.181Gbp of sequence, which is exactly equivalent to the length of the input reference plus the length of the novel alleles in the VCF file.
...
We also mapped a real human genome read set with $\sim$50× coverage of Illumina 150bp paired-end reads from the NA24385 sample to the 1000GP graph. {\tt vg} produced mappings for 98.7\% of the reads, 88.7\%
...
\begin{figure}[htbp!]
  ...
  \caption[{\tt vg msga} progressive alignment of the MHC in GRCh38.]{
    Variation graph of the MHC in GRCh38 constructed by progressive alignment with {\tt vg msga} and visualized by Bandage.
  }
  \label{fig:vg_msga_mhc}
\end{figure}

To appreciate the effect of repeat collapse on the resulting graph, I applied {\tt vg dotplot}, which generates path-coincidence dotplots from indexed variation graphs, to compare the in-graph alignments of the same pair of sequences in both the {\tt seqwish} and {\tt vg msga} graphs.
These results are presented in figure \ref{fig:mhc_dotplots}.

{\tt vg dotplot} does not make a dotplot in the same manner as is done to represent pairwise alignments.
...
For a given pair of paths embedded in the graph, {\tt vg dotplot} renders a dot for each instance where both paths traverse the same graph position.
...
{\tt minimap2} allows for the multiple mapping of parts of its query sequences, but no filtering was completed on the input alignments to reduce the impact of multimapping on the resulting graph.
In contrast, figure \ref{subfig:vg_msga_mhc_dotplot} shows that a different alignment model allow the sequences to align through the graph in an approximately linear fashion, with little repeat collapse.
...
\begin{figure}[htbp!]
  ...
  \end{subfigure}
  \caption[Path-coincidence dotplots from variation graphs of the MHC in GRCh38.]{
    Comparing {\tt seqwish} and {\tt vg msga} variation graphs built for the MHC in GRCh38.
    In both panels we see a path-coincidence dotplot between GI568335954 ($x$-axis) and GI568335994 ($y$-axis) as they are embedded in each graph, with \ref{subfig:seqwish_mhc_dotplot} showing the plot for {\tt seqwish} and \ref{subfig:vg_msga_mhc_dotplot} that for {\tt vg msga}.
    In these plots, we render a dot for each pair of path positions that coincide in the graph.
    The repeated positions in the {\tt seqwish} plot, which appear as dark verticals banded regions in the plot, represent repeats that have triggered the collapse of multiple positions in each path onto the same graph position.
    In contrast, the {\tt vg msga} plot shows a relatively linear relationship between the two sequences in the graph, with large indels and a few gaps, but no off-diagonal relationships that might suggest repeat collapse or structural variation.
  }
...
We compared mapping with {\tt bwa mem} and {\tt vg} for data set ENCFF000ATK from the ENCODE project \cite{encode2012integrated}, which contains 14.9 million 51bp ChIP-seq reads for the H3K4me1 histone methylation mark from the NA12878 cell line.
When mapping with bwa the ratio of reference to alternate allele matches at heterozygous sites was 1.20, whereas with {\tt vg} to the 1000GP graph the ratio was 1.01, effectively eliminating reference bias.
...
\begin{figure}[htbp!] 
  ...
  \end{subfigure}
  \caption[Resolving reference bias in 36bp CHiP-seq]{
    Resolving reference bias in 36bp CHiP-seq data from ENCODE sample ENCDO115AAA, using CHiP-seq targeting H3K4me1 (experiment ENCFF000ATK).
...
The effect is dramatically stronger when using 36bp reads.
...
\begin{figure}[htbp!]
  ...
  \begin{subfigure}[t]{1.0\textwidth}
    ...
    \caption{Per-strain coverage across 100kbp of the \emph{E. coli} pangenome}
...
\begin{figure}[htbp!]
  ...
  \caption[An arctic freshwater viral metagenome]{
    An arctic freshwater viral metagenome graph, built with {\tt minia} as described in the text.
    As in other Bandage visualizations, colored splines in this figure represent contigs or nodes in the assembly graph, with edges represented by junctions connecting the ends of these splines.
    The width of the nodes corresponds to the coverage measured by {\tt minia} during the assembly process.
    ...
    We should expect these to represent fully assembled viral genomes \cite{de2015biodiversity}.
    ...
    We can see that there are few very short contigs (at the bottom of the rendering) with high coverage, which may indicate that these represent partial assemblies, rare species, or contaminants from non-viral sources.
  }
...
\begin{figure}[htbp!]
  ...
  \caption[Comparing {\tt vg} and {\tt bwa} alignment to the viral metagenome]{
    ...
    The region of higher density seen below the diagonal in the upper right of the plot indicates an increase in alignment score when mapping against the full assembly graph (with {\tt vg map}) relative to mapping to the linearized contig set (with {\tt bwa mem}).
  }
