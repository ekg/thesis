...
In each read the SNP position was mutated to the alternate allele.
Reads with the base found in the reference genome and reads with the alternate base were mapped back to the 1000GP graph or GRCh37 linear genome using {\tt vg map} and {\tt bwa aln} respectively, with {\tt vg}'s surjection process used to produce a BAM file from the alignment for direct comparison.
...
This effect is notable at damage rates as low as 10\%, and with 30\% PMD error the rate of alignment to non-reference alleles is reduced by nearly 15\%.
...
\begin{figure}[htbp!]
  ...
  \caption[Allele balance in the Yamnaya sample]{
    Allele balance in the Yamnaya sample versus presence in the 1000GP graph.
...
To systematically evaluate if alignment against the pangenome would reduce reference bias, we apply the ABBA BABA test of phylogenetic tree topology based on Patterson's D statistic of admixture \cite{green2010draft}.

\begin{figure}[htbp!]
  \centering
  \begin{subfigure}[t]{0.49\textwidth}
    \includegraphics[width=1.0\textwidth]{Chapter3/Figs/aDNA_d_stats_absolute.pdf}
    \caption{D-stats absolute values}
    \label{subfig:aDNA_dstats_absolute}
  \end{subfigure}
  \begin{subfigure}[t]{0.49\textwidth}
    \includegraphics[width=1.0\textwidth]{Chapter3/Figs/aDNA_d_stats_z_score.pdf}
    \caption{D-stats z-scores}
    \label{subfig:aDNA_dstats_zscore}
  \end{subfigure}
  \caption[Bias towards the reference genome in aDNA]{
    xxx
  }
...
\begin{comment}
  ...
  - Used the Human Origins dataset distributed with https://www.nature.com/articles/nature19310#ref13
  \cite{lazaridis2014ancient}
-  for each sample:
- ran samtools pileup, calling ~1.2 million SNPs
- converted pileup to plink format, selecting one allele at random, and converting each sample to homozygous for the selected allele. This was done 5 times (5 replicates of randomly selected alleles for each sample)
- Selected the Href/hg19 and Chimp sample from the dataset above and filtered by genotyping rate, excluding poorly genotyped SNPs (--geno 0.05), resulting in 1142867 SNPs
- merged individual plink files with hg19 and chimp
- converted plink to eigenstrat format using convertf and ran D-stats using qpDstat (Patterson
 et al. (2012) Ancient Admixture in Human History) of the form D(vg, bwa; GRCh37,Chimp) to test for excess of shared alleles between vg and bwa aligned samples with the reference.
\end{comment}
...
%ecoli_10strain_kmer51abund10_contigs_vg_gfa.png
\begin{figure}[htbp!]
  \centering
...
\begin{figure}[htbp!]
  \centering
  \includegraphics[width=1.0\textwidth]{Chapter3/Figs/ecoli_10strain_cov_col_pos_0_1M.png}
  \caption{
    ...
  }
  \label{fig:ecoli_cov_strains}
\end{figure}

\begin{figure}[htbp!]
  \centering
  \includegraphics[width=0.7\textwidth]{Chapter3/Figs/ecoli_cov_strains_hist.pdf}
  \caption{
    ...
  }
  \label{fig:ecoli_cov_strains}
...
\begin{figure}[htbp!]
  \centering
  \begin{subfigure}[t]{1.0\textwidth}
    \includegraphics[width=1.0\textwidth]{Chapter3/Figs/raw_viral_metagenome_zoom.png}
    \caption{A subset of the raw viral metagenome assembly graph}
  \end{subfigure}
  \begin{subfigure}[t]{1.0\textwidth}
    \includegraphics[width=1.0\textwidth]{Chapter3/Figs/smoothed_viral_metagenome_zoom.png}
    \caption{A subset of the smoothed viral metagenome assembly graph}
  \end{subfigure}
  \caption{
    ...
  }
...
Presumably the two ends of dumbell-like main component of the graph represent these phyla, with the connection between them generated by horizontal transfer of genes or spurious correlation due to recurrent $k$-mers in the assembly graph.

\begin{figure}[htbp!]
  \centering
  \includegraphics[width=1.0\textwidth]{Chapter3/Figs/SRS105153_k51a3_contigs_gfa.png}
  \caption{
    ...
  }
  \label{fig:human_gut_microbiome}
\end{figure}
