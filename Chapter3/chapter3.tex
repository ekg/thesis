...
\begin{figure}[htbp!]
  ...
  \caption[Cactus yeast simulation]{Mapping short reads with \DIFaddbeginFL {\tt \DIFaddendFL vg\DIFaddbeginFL } \DIFaddendFL to yeast genome references.
    ROC curves obtained by mapping 100,000 simulated SK1 yeast strain \DIFdelbeginFL \DIFdelFL{150-bp }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{150bp }\DIFaddendFL paired-end reads against a variety of references described in the text.
    Reprinted from \cite{garrison2018variation}.}
  \label{fig:cactus_yeast_sim}
\end{figure}


We simulated 100,000 \DIFdelbegin \DIFdel{150-bp }\DIFdelend \DIFaddbegin \DIFadd{150bp }\DIFaddend paired-end reads from the SK1 reference, modeling sequencing errors, and mapped them to the four references (ROC curves, Figure \ref{fig:cactus_yeast_sim}).
...
The SGD\_2010 reference contains the mitochondria and \DIFdelbegin \DIFdel{6 kbp }\DIFdelend \DIFaddbegin \DIFadd{6kbp }\DIFaddend plasmid sequence, while the S288c assembly excludes them due to the sequencing protocol, where size selection in the library preparation stage removed these short sequences, so to enable direct comparison in later tests these were added to the S288c graph.
...
For a species such as human, with only 0.1\% nucleotide divergence on average between individual genome sequences, over 90\% of \DIFdelbegin \DIFdel{100-bp }\DIFdelend \DIFaddbegin \DIFadd{100bp }\DIFaddend reads will derive from sequence exactly matching the reference.
...
We also mapped a real human genome read set with $\sim$50× coverage of Illumina \DIFdelbegin \DIFdel{150-bp }\DIFdelend \DIFaddbegin \DIFadd{150bp }\DIFaddend paired-end reads from the NA24385 sample to the 1000GP graph. {\tt vg} produced mappings for 98.7\% of the reads, 88.7\%
...
We compared mapping with {\tt bwa mem} and {\tt vg} for data set ENCFF000ATK from the ENCODE project \cite{encode2012integrated}, which contains 14.9 million \DIFdelbegin \DIFdel{51-bp }\DIFdelend \DIFaddbegin \DIFadd{51bp }\DIFaddend ChIP-seq reads for the H3K4me1 histone methylation mark from the NA12878 cell line.
...
\begin{figure}[htbp!] 
  ...
  \end{subfigure}
  \DIFdelbeginFL %DIFDELCMD < \caption[Resolving reference bias in 36-bp CHiP-seq]{%%%
\DIFdelendFL \DIFaddbeginFL \caption[Resolving reference bias in 36bp CHiP-seq]{\DIFaddendFL 
    Resolving reference bias in \DIFdelbeginFL \DIFdelFL{36-bp }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{36bp }\DIFaddendFL CHiP-seq data from ENCODE sample ENCDO115AAA, using CHiP-seq targeting H3K4me1 (experiment ENCFF000ATK).
...
The effect is dramatically stronger when using \DIFdelbegin \DIFdel{36-bp }\DIFdelend \DIFaddbegin \DIFadd{36bp }\DIFaddend reads.
...
\begin{figure}[htbp!]
  ...
  \begin{subfigure}[t]{1.0\textwidth}
    ...
    \caption{Per-strain coverage across \DIFdelbeginFL \DIFdelFL{100 kbp }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{100kbp }\DIFaddendFL of the \emph{E. coli} pangenome}
...
\begin{figure}[htbp!]
  ...
  \caption[An arctic freshwater viral metagenome]{
    ...
    }\DIFaddendFL Bandage \DIFaddbeginFL \DIFaddFL{renders graphs with many small contigs with a vertical order related to the size of each weakly connected component, so the nodes at the bottom are smaller than those at the top.
